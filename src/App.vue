<template>
	<Modal
		...
		<div v-if="currentSlide !== 0 || !withIntro" class="modal-header">
			<div class="nmcfirstrunwizard-header">
				<div class="logo">
		...
		<div class="modal-footer">
			<button v-if="isLast" class="primary modal-default-button" @click="close">
				{{ t('nmcfirstrunwizard', 'Start using Nextcloud') }}
			</button>
		</div>
	</Modal>
</template>
<style lang="scss">
	...
	@media only screen and (max-width: 680px) {
		#nmcfirstrunwizard {
			.nmcfirstrunwizard-header div.logo {
...
export default {
	...
	methods: {
		async loadStaticSlides() {
			...
			try {
				const response = await axios.get(generateUrl('/apps/nmc-firstrunwizard/wizard'))
		...
		close() {
			...
			axios.delete(generateUrl('/apps/nmc-firstrunwizard/wizard'))
		},
		next() {
